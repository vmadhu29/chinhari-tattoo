...
<section class="relative pt-32 pb-16 overflow-hidden" aria-label="Portfolio Header">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        <div class="flex items-center justify-center gap-4 mb-4">
            ...
            <span class="text-verli text-xs tracking-[0.4em] uppercase font-semibold">Tattoo Gallery</span>
            <div class="w-8 h-px bg-verli/60"></div>
        </div>
        <h1 class="font-display text-5xl md:text-7xl text-studio-white tracking-wider mb-6">
            DESIGN <span class="text-verli-gradient">PORTFOLIO</span>
        </h1>
        ...
        <div class="flex flex-wrap items-center justify-center gap-2">
            <a 
                ...
                class="px-5 py-2 text-xs font-semibold uppercase tracking-wider rounded-full border transition-all duration-200
                       {{ !$selectedStyle ? 'bg-verli border-verli text-studio-black' : 'border-studio-border text-studio-muted hover:text-studio-white hover:border-studio-muted bg-studio-black' }}"
            >
                All Works
            </a>
            @foreach($categories as $cat)
                <a 
                    ...
                    class="px-5 py-2 text-xs font-semibold uppercase tracking-wider rounded-full border transition-all duration-200
                           {{ $selectedStyle === $cat->slug ? 'bg-verli border-verli text-studio-black' : 'border-studio-border text-studio-muted hover:text-studio-white hover:border-studio-muted bg-studio-black' }}"
                >
                    {{ $cat->name }}
                </a>
            @endforeach
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach($portfolios as $i => $item)
                    <a 
                        ...
                        class="portfolio-item rounded-2xl overflow-hidden aspect-[3/4] relative block group border border-studio-border/30 hover:border-verli/30 transition-all duration-300"
                        data-reveal
                        data-delay="{{ ($i % 8) * 50 }}"
