<div class="max-w-4xl mx-auto px-4 py-8 relative">
    ...
    <div class="mb-12" aria-label="Booking Progress">
        <div class="flex items-center justify-between relative">
            ...
            {{-- Active track line --}}
            <div class="absolute left-0 top-1/2 -translate-y-1/2 h-0.5 bg-gold transition-all duration-500 z-0" 
            ...
            @foreach([
                ...
                <div class="flex flex-col items-center relative z-10">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300
                                {{ $step === $s ? 'bg-gold border-gold text-studio-black shadow-gold-sm font-semibold' : ($step > $s ? 'bg-studio-black border-gold text-gold font-medium' : 'bg-studio-black border-studio-border text-studio-muted') }}">
                        <span class="text-sm">{{ $info['icon'] }}</span>
                    </div>
                    <span class="text-[10px] uppercase tracking-wider mt-2 font-medium hidden sm:block
                                 {{ $step === $s ? 'text-gold' : ($step > $s ? 'text-studio-white' : 'text-studio-faint') }}">
                        {{ $info['label'] }}
                    </span>
                </div>
            @endforeach
                            ...
                            @foreach($categories as $cat)
                                <button 
                                    ...
                                    class="p-4 rounded-xl border text-center flex flex-col items-center justify-center gap-2 transition-all duration-300
                                           {{ $selectedCategory == $cat->id ? 'border-gold bg-gold/5 text-gold shadow-gold-sm' : 'border-studio-border bg-studio-black text-studio-gray hover:border-studio-muted hover:text-white' }}"
                                >
                                ...
                                @foreach($services as $serv)
                                    <button 
                                        ...
                                        class="p-4 rounded-xl border text-left flex flex-col justify-between gap-2 transition-all duration-300 bg-studio-black
                                               {{ $selectedService == $serv->id ? 'border-gold bg-gold/5 text-gold' : 'border-studio-border text-studio-gray hover:border-studio-muted hover:text-white' }}"
                                    >
                                        <div>
                                            <h4 class="font-semibold text-sm {{ $selectedService == $serv->id ? 'text-gold' : 'text-studio-white' }}">{{ $serv->name }}</h4>
                                        ...
                                        <div class="flex justify-between items-center w-full pt-3 border-t border-studio-border/20 text-xs">
                                            ...
                                            <span class="font-semibold text-gold">Est. {{ $serv->estimated_duration_minutes }} mins</span>
                                        </div>
                                    </button>
                                @endforeach
                            </div>
                            ...
                            @foreach($artists as $art)
                                <button 
                                    ...
                                    class="p-4 rounded-xl border text-left flex items-center gap-4 transition-all duration-300 bg-studio-black
                                           {{ $selectedArtist == $art->id ? 'border-gold bg-gold/5 text-gold' : 'border-studio-border text-studio-gray hover:border-studio-muted hover:text-white' }}"
                                >
                                    <img src="{{ $art->profile_photo_url }}" alt="{{ $art->display_name }}" class="w-12 h-12 rounded-full object-cover border border-studio-border">
                                    <div>
                                        <h4 class="font-semibold text-sm {{ $selectedArtist == $art->id ? 'text-gold' : 'text-studio-white' }}">{{ $art->display_name }}</h4>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <input 
                                ...
                                class="w-full bg-studio-black border border-studio-border rounded-xl px-4 py-3.5 text-sm text-studio-white focus:outline-none focus:border-gold"
                            >
                                    ...
                                    @foreach($availableSlots as $slot)
                                        <button 
                                            ...
                                            class="py-3.5 text-center text-xs font-semibold rounded-lg border transition-all duration-200 bg-studio-black
                                                   {{ $selectedTime === $slot['start'] ? 'border-gold bg-gold text-studio-black font-bold' : 'border-studio-border text-studio-gray hover:border-studio-muted hover:text-white' }}"
                                        >
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="form-name" wire:model.blur="name" required class="w-full bg-studio-black border border-studio-border rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-gold">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="email" id="form-email" wire:model.blur="email" required class="w-full bg-studio-black border border-studio-border rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-gold">
                                ...
                                <div>
                                    ...
                                    <input type="tel" id="form-phone" wire:model.blur="phone" required class="w-full bg-studio-black border border-studio-border rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-gold">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" id="form-placement" wire:model.blur="placement" placeholder="e.g. Forearm, Chest" required class="w-full bg-studio-black border border-studio-border rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-gold">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="form-size" wire:model.blur="size" placeholder="e.g. 4x4 inches" required class="w-full bg-studio-black border border-studio-border rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-gold">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <button type="button" wire:click="$set('color_type', 'black-grey')" class="py-2.5 rounded-lg border text-xs font-semibold transition-all duration-200 bg-studio-black {{ $color_type === 'black-grey' ? 'border-gold bg-gold/5 text-gold' : 'border-studio-border text-studio-muted' }}">Black & Grey</button>
                                    <button type="button" wire:click="$set('color_type', 'color')" class="py-2.5 rounded-lg border text-xs font-semibold transition-all duration-200 bg-studio-black {{ $color_type === 'color' ? 'border-gold bg-gold/5 text-gold' : 'border-studio-border text-studio-muted' }}">Colored Ink</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <textarea id="form-desc" wire:model.blur="description" rows="3" required placeholder="Tell us the story behind it, reference details, style modifications..." class="w-full bg-studio-black border border-studio-border rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-gold"></textarea>
                            ...
                            <div class="space-y-3 pt-3">
                                <label class="flex items-start gap-3 cursor-pointer">
                                    <input type="checkbox" wire:model.live="consent_age" class="mt-1 border-studio-border rounded text-gold focus:ring-0 focus:ring-offset-0 bg-studio-black">
                                ...
                                <label class="flex items-start gap-3 cursor-pointer">
                                    <input type="checkbox" wire:model.live="consent_health" class="mt-1 border-studio-border rounded text-gold focus:ring-0 focus:ring-offset-0 bg-studio-black">
            ...
            @if($step === 4)
                <div class="space-y-8 animate-fade-in" x-data="{
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="card p-6 bg-studio-black border border-studio-border rounded-2xl flex flex-col justify-between text-center">
                            <div>
                                <span class="text-gold text-2xl font-bold uppercase block tracking-wider mb-2">Slot Reservation</span>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="text-gold font-display text-5xl font-semibold">₹{{ number_format($deposit_amount) }}</div>
                                ...
                                @if($razorpay_order_id)
                                    <button 
                                        type="button" 
                                        @click="initRazorpay()" 
                                        class="btn-gold w-full py-4 text-center text-sm font-semibold rounded-xl mt-4 flex items-center justify-center gap-3"
                                    >
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                                        Pay Deposit & Book
                                    </button>
                                @else
                                    <div class="py-3 text-center text-xs text-gold animate-pulse">Initializing Payment Order...</div>
                                @endif
        ...
        <div class="flex justify-between items-center pt-8 border-t border-studio-border/30 mt-10">
            ...
            @if($step < 4)
                <button 
                    type="button" 
                    wire:click="nextStep" 
                    class="btn-gold px-8 py-3 flex items-center gap-2 text-sm"
                >
                    Continue
